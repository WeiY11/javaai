        ...
        try (Workbook workbook = WorkbookFactory.create(filePath.toFile(), (String) null, true)) {
            ...
            for (int i = 0; i < sheetCount; i++) {
                ...
                for (int r = 1; r <= sheet.getLastRowNum(); r++) {
                    ...
                    for (int c = 0; c < colCount; c++) {
                        ...
                        if (cell != null && cell.getCellType() == CellType.NUMERIC) {
                            numericCols.computeIfAbsent(c, k -> new ArrayList<>()).add(cell.getNumericCellValue());
                        }
                    }
                }
                for (Map.Entry<Integer, List<Double>> entry : numericCols.entrySet()) {
                    ...
                    String colName = headerRow != null ? getCellStringValue(headerRow.getCell(colIdx)) : ("Col" + colIdx);
                    double min = values.stream().mapToDouble(Double::doubleValue).min().orElse(0);
                    double max = values.stream().mapToDouble(Double::doubleValue).max().orElse(0);
                    double avg = values.stream().mapToDouble(Double::doubleValue).average().orElse(0);
                    sb.append("- ").append(colName).append(": min=").append(String.format("%.2f", min))
                      .append(", max=").append(String.format("%.2f", max))
            ...
            return ExtractionResult.success(content, "structured-text",
                Map.of("sheetCount", sheetCount, "totalRows", totalRows));
