...
public class EvidencePortfolioSelector {
  ...
  public List<SearchResult> select(
      String query, List<SearchResult> candidates, int maxContextChars) {
    if (candidates == null || candidates.isEmpty()) {
      return List.of();
    }

    int budget = Math.max(MIN_CONTEXT_BUDGET, maxContextChars);
    List<CandidateFeature> pool =
        deduplicate(candidates).stream()
            .sorted(Comparator.comparingDouble(SearchResult::getScore).reversed())
            .map(
                result ->
                    new CandidateFeature(
                        result, tokenize(result.getContent()), estimateBlockChars(result)))
            .toList();
    ...
    while (selected.size() < pool.size()) {
      ...
      for (CandidateFeature candidate : pool) {
        ...
        CandidateScore score =
            scoreCandidate(candidate, selected, selectedDocs, queryTerms, coveredTerms);
        if (!selected.isEmpty()
            && !queryTerms.isEmpty()
  ...
  private CandidateScore scoreCandidate(
    ...
    double value =
        0.62 * confidence + 0.22 * coverageGain + 0.12 * diversityBonus - 0.24 * redundancyPenalty;
    return new CandidateScore(candidate, value, coverageGain, diversityBonus);
  }

  private double coverageGain(
      CandidateFeature candidate, Set<String> queryTerms, Set<String> coveredTerms) {
  ...
  private record CandidateScore(
      CandidateFeature result, double value, double coverageGain, double diversityBonus) {}
}
