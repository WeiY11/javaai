...
public class DataCleanupExecutor implements TaskExecutor {
  ...
  public void execute(ScheduledTask task) {
    try {
      ...
      int retentionDays =
          config.get("retentionDays") != null
              ? Integer.parseInt(config.get("retentionDays").toString())
              : 90;

      LocalDateTime cutoff = LocalDateTime.now().minusDays(retentionDays);
      log.info(
          "DATA_CLEANUP: cleaning conversations older than {} days (before {})",
          retentionDays,
          cutoff);

      List<Conversation> oldConversations =
          conversationMapper.selectList(
              new LambdaQueryWrapper<Conversation>().lt(Conversation::getCreatedAt, cutoff));
      ...
      List<Long> convIds =
          oldConversations.stream().map(Conversation::getId).collect(Collectors.toList());
      ...
      long deletedConversations =
          conversationMapper.delete(
              new LambdaQueryWrapper<Conversation>().in(Conversation::getId, convIds));

      log.info(
          "DATA_CLEANUP: deleted {} conversations and {} messages",
