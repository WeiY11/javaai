...
public class AiConfig {
    ...
    if (aiProperties.getProviders() != null) {
      for (Map.Entry<String, AiProperties.ProviderConfig> entry :
        ...
        try {
          ...
          org.springframework.web.reactive.function.client.WebClient.Builder wcBuilder =
                              ...
                              return new com.fasterxml.jackson.databind.JsonSerializer<Object>() {
                                ...
                                public void serialize(
                                  ...
                                  ((com.fasterxml.jackson.databind.JsonSerializer<Object>)
                                          serializer)
                                      .serialize(value, tb, serializers);

                                  com.fasterxml.jackson.databind.JsonNode rootNode =
                                      mapper.readTree(tb.asParser());
                                  if (rootNode.isObject() && rootNode.has("model")) {
                                    String modelStr = rootNode.get("model").asText();
                                    if (modelStr != null
                                        && modelStr.contains("|thinking:enabled")) {
                                      ...
                                      com.fasterxml.jackson.databind.node.ObjectNode thinkingNode =
                    ...
                    .codecs(
                        configurer -> {
                          configurer
                              .defaultCodecs()
                              .jackson2JsonEncoder(
                                  new org.springframework.http.codec.json.Jackson2JsonEncoder(
                                      mapper));
                          configurer
                              .defaultCodecs()
                              .jackson2JsonDecoder(
          ...
          OpenAiApi openAiApi =
              new OpenAiApi(config.getBaseUrl(), config.getApiKey(), rcBuilder, wcBuilder);

          OpenAiChatOptions options =
              OpenAiChatOptions.builder()
                  .withModel(config.getModel())
                  .withTemperature(
                      config.getTemperature() != null ? config.getTemperature().floatValue() : 0.7f)
                  .build();
          ...
          LoggerFactory.getLogger(AiConfig.class)
              .warn(
                  "Failed to create ChatClient for provider '{}': {}",
                  providerName,
                  e.getMessage());
        }
      }
  ...
  public OpenAiEmbeddingModel embeddingModel() {
    ...
    OpenAiEmbeddingModel embeddingModel =
        new OpenAiEmbeddingModel(embeddingApi, MetadataMode.EMBED);
    LoggerFactory.getLogger(AiConfig.class)
        .info(
            "Embedding model created: model={}, dimension={}",
            model,
