...
public class DeepSeekExchangeFilterFunction implements ExchangeFilterFunction {
  ...
  public Mono<ClientResponse> filter(ClientRequest request, ExchangeFunction next) {
    if (request.url().getHost() != null
        && request.url().getHost().contains("deepseek")
        && request.method() == HttpMethod.POST) {
      // Spring WebClient request body is hard to intercept and modify synchronously.
      // But Spring AI provides a way to customize request bodies, or we can use a custom request
      // interceptor.
      // Since modifying WebClient bodies requires intercepting Publisher streams, it's non-trivial.
      // Is there an easier way?
      // Actually, we can use a ClientHttpRequestInterceptor if Spring AI is using RestClient for
      // streams? No, Spring AI uses WebClient for streams.
