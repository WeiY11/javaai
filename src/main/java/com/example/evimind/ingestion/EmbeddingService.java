...
public class EmbeddingService {
  ...
  @Autowired private com.example.evimind.mapper.DocumentChunkEmbeddingMapper embeddingMapper;
  ...
  public void embedAndStore(List<DocumentChunk> chunks) {
    ...
    for (int i = 0; i < chunks.size(); i += batchSize) {
      ...
      List<String> texts =
          batchChunks.stream().map(DocumentChunk::getContent).collect(Collectors.toList());
