 ...
 *
 * 将查询与候选文本以 pair 方式发送给 LLM，请求其为每个候选打分（0-10 的相关性分数）。
 * 采用批量 prompt 策略：一次 LLM 调用评估所有候选，减少 API 调用次数和延迟。
 *
 * 面试点：
 * - Cross-Encoder 思路：query + candidate 联合输入，比 Bi-Encoder 的独立编码更准确
 * - 批量评估 vs 逐条评估的 latency-cost tradeoff
 * - 优雅降级：Reranker 失败时保持原始 RRF 排序不变
 */
@Slf4j
@Component
@ConditionalOnProperty(name = "custom.rag.reranker.type", havingValue = "prompt", matchIfMissing = true)
public class PromptBasedReranker implements Reranker {
    ...
    public List<SearchResult> rerank(String query, List<SearchResult> candidates, int topN) {
        ...
        // 限制候选数量，避免 prompt 过长
        List<SearchResult> inputCandidates = candidates.size() > maxCandidates
        ...
        try {
            ...
            if (scores != null && scores.size() == inputCandidates.size()) {
                ...
                log.info("Reranker rescored {} candidates, top score: {} -> {}",
