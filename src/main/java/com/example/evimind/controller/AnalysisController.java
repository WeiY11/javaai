...
import com.example.evimind.model.BatchAnalysisItemResult;
import com.example.evimind.model.BatchAnalysisRequest;
import com.example.evimind.model.BatchProgress;
import com.example.evimind.model.dto.ApiResponse;
import com.example.evimind.service.*;
...
public class AnalysisController {
  ...
  private final ConcurrentHashMap<String, List<BatchAnalysisItemResult>> batchResults =
          ...
          for (String filePath : finalPaths) {
            ...
            try {
              ...
              if (content.length() > analysisProperties.getMaxPromptSize()) {
                content =
                    content.substring(0, analysisProperties.getMaxPromptSize()) + "\n... [内容已截断]";
              }
  ...
  @PostMapping("/batch")
  public ApiResponse<Map<String, String>> startBatchAnalysis(
      @RequestBody BatchAnalysisRequest request) throws IOException {
  ...
  public ResponseEntity<InputStreamResource> exportMarkdown(
      ...
      @RequestParam(value = "title", defaultValue = "分析报告") String title)
      throws IOException {
