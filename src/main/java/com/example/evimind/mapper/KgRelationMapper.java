...
public interface KgRelationMapper extends BaseMapper<KgRelation> {
  ...
  @Select(
      "SELECT * FROM kg_relation WHERE knowledge_base_id = #{knowledgeBaseId} ORDER BY created_at")
  ...
  @Select(
      "SELECT * FROM kg_relation WHERE source_entity_id = #{entityId} OR target_entity_id = #{entityId}")
