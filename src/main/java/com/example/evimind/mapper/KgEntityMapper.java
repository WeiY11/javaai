...
public interface KgEntityMapper extends BaseMapper<KgEntity> {
    ...
    @Select("SELECT * FROM kg_entity WHERE knowledge_base_id = #{knowledgeBaseId} ORDER BY created_at")
    ...
    @Select("SELECT * FROM kg_entity WHERE name = #{name} AND knowledge_base_id = #{knowledgeBaseId} LIMIT 1")
    KgEntity findByNameAndKb(@Param("name") String name, @Param("knowledgeBaseId") Long knowledgeBaseId);
