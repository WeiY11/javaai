...
public interface DocumentPermissionMapper extends BaseMapper<DocumentPermission> {
    ...
    @Select("SELECT * FROM document_permission WHERE document_id = #{documentId} AND user_id = #{userId}")
    List<DocumentPermission> findByDocumentAndUser(@Param("documentId") Long documentId,
    ...
    @Select("""
        ...
        """)
    List<Long> findPermittedDocumentIds(@Param("userId") Long userId,
