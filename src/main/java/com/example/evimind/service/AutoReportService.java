...
/**
 * 自动化报告生成服务。
 * 基于知识库的对话历史、文档分析结果和引用网络，生成结构化周报/月报。
 */
@Slf4j
@Service
...
public class AutoReportService {
    ...
    public String generateReport(Long knowledgeBaseId, String period) {
        ...
        if (kb == null) throw new IllegalArgumentException("Knowledge base not found: " + knowledgeBaseId);

        LocalDateTime since = "monthly".equals(period)
    ...
    private Map<String, Object> collectStats(Long knowledgeBaseId, LocalDateTime since) {
        ...
        long newDocs = documentMapper.selectCount(
        ...
        List<Document> docs = documentMapper.selectList(
                new LambdaQueryWrapper<Document>().eq(Document::getKnowledgeBaseId, knowledgeBaseId));
        Map<String, Long> formatDist = docs.stream()
        ...
        return """
            ...
            *报告由系统自动生成，LLM 服务不可用时使用统计模板。*
            """.formatted(
                kbName, periodLabel,
                stats.get("period"),
                stats.get("totalDocuments"), stats.get("newDocuments"),
                stats.get("newConversations"),
                stats.get("totalMessages"),
                stats.get("formatDistribution")
        );
