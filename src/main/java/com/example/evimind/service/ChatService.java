...
public class ChatService {
    ...
    public Flux<String> streamChat(String provider, String message, String sessionId) {
        ...
        return baseClient.mutate()
                ...
                .build()
                .prompt()
                .system("你是一个具有本地数据文件管理、内容检索和历史分析结果查询能力的智能 AI 助手。你可以而且应该优先使用提供的工具函数（如 listDirectory, readFileContent, queryAnalysisHistory）来获取本地文件的实际状态和内容，然后再给用户作答。不要凭空捏造文件列表或内容。")
                .user(message)
                .stream()
                .content();
    }
