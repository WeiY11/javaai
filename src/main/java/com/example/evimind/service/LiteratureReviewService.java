...
public class LiteratureReviewService {
  ...
  public String generateReview(Long knowledgeBaseId, String topic) {
    ...
    Map<Long, List<DocumentChunk>> chunksByDoc =
        chunks.stream().collect(Collectors.groupingBy(DocumentChunk::getDocumentId));
    ...
    try {
      ...
      log.info(
          "Literature review generated successfully for topic '{}' in KB {}",
          topic,
          knowledgeBaseId);
  ...
  /** 构建论文信息文本，包含元数据和内容摘要。 */
  private String buildPapersInfo(
      List<Document> papers, Map<Long, List<DocumentChunk>> chunksByDoc) {
    ...
    for (int i = 0; i < papers.size(); i++) {
      ...
      List<DocumentChunk> docChunks =
          chunksByDoc.getOrDefault(doc.getId(), Collections.emptyList());
