...
/** QueryRewriteService 单元测试。 覆盖核心场景：空历史跳过、正常改写、改写失败降级、历史截断、超时处理。 */
...
class QueryRewriteServiceTest {
  ...
  void shouldReturnRewrittenQuery() {
    ...
    when(chatClient.prompt().user(anyString()).call().content()).thenReturn("机器学习的优点是什么");
  ...
  void shouldTruncateLongHistory() {
    ...
    when(promptTemplateManager.render(eq("query-rewrite-prompt"), anyMap()))
        .thenReturn("rewrite prompt");
    when(chatClient.prompt().user(anyString()).call().content()).thenReturn("rewritten");
    ...
    verify(promptTemplateManager)
        .render(
            eq("query-rewrite-prompt"),
            argThat(
                vars -> {
