...
class RrfFusionServiceTest {
    ...
    void shouldReturnThresholdFriendlyConfidenceScores() {
        ...
        assertEquals("c1", fused.get(0).getChunkId());
        assertTrue(fused.get(0).getScore() >= 0.80, "agreed top evidence should clear a 0.50 threshold comfortably");
    ...
    void shouldMergeSameDocumentChunkEvenWhenBackendIdsDiffer() {
        List<SearchResult> semantic = List.of(
                new SearchResult("chunk_100", 10L, 1L, "same chunk", 3, 0.95, "pgvector")
        );
        List<SearchResult> keyword = List.of(
                new SearchResult("chunk_10_3", 10L, 1L, "same chunk", 3, 5.0, "elasticsearch")
        );
